...
export function authenticate(sessionId: string): Promise<string> {
  return fetch(buildUrl(PATHS.accountNameUrl), sessionId).then(
    (resp) => JSON.parse(resp).accountName,
  );
}

export function loadCharacters(sessionId: string) {
  return fetch(buildUrl(PATHS.charactersUrl), sessionId).then(
    (resp) => JSON.parse(resp) as Character[],
  );
}

export function loadInventory(sessionId: string, character: string, accountName: string) {
  return fetch(buildUrl(PATHS.inventoryUrl), sessionId, 'POST', {
    character,
    accountName,
...
export function loadPassiveSkills(sessionId: string, character: string, accountName: string) {
  return fetch(buildUrl(PATHS.passiveTreeUrl, { character, accountName }), sessionId).then((resp) =>
    (JSON.parse(resp) as { items: RawItem[] }).items.map(decorateItem),
  );
}
...
export function loadStash(
  ...
  return fetch(buildUrl(PATHS.stashUrl, query), sessionId).then((resp) =>
    (JSON.parse(resp) as { items: RawItem[] }).items.map(decorateItem),
  );
}
...
export function loadLeagueStashInformation(sessionId: string, league: string, accountName: string) {
  ...
  return fetch(buildUrl(PATHS.stashMetadataUrl, query), sessionId).then(
    (resp) => JSON.parse(resp).tabs as StashPage[],
  );
}
