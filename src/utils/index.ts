...
const booleanValueOf = (option: FilterBooleanOptions) => {
  switch (option) {
    case FilterBooleanOptions.YES:
      return true;
    case FilterBooleanOptions.NO:
      return false;
    default:
      return undefined;
  }
...
export const matchItemCategory = (item: Item, value: any): boolean => {
  if (category[value as ItemCategory]) {
    ...
    return groupedCategory[value as GroupedItemCategory].some((itemCategory) =>
      matchItemCategory(item, itemCategory),
    );
  }
  return false;
};
