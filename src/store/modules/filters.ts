 ...
 * Getters *
 ************/
const getIndexerFilter = builder.read(
  (state) => (type: string, serial: number = 0) =>
    state.indexerFilters.find((filter) => filter.type === type && filter.serial === serial),
  'getIndexerFilter',
);

const getFunctionalFilter = builder.read(
  (state) => (type: string) => state.functionalFilters.find((filter) => filter.type === type),
  'getFunctionalFilter',
);

const getFilter = builder.read(
  () => (type: string, serial?: number) =>
    getIndexerFilter()(type, serial) || getFunctionalFilter()(type),
  'getFilter',
);
...
const filterItems = builder.dispatch(async () => {
  ...
  if (validIndexerFilters.length) {
    validIndexerFilters.forEach((indexerFilter, i) => {
      ...
      const filteredResult = itemStore.queryIndexerResults(
        indexerFilter.type,
        keyword === OPTION_ANY ? '' : keyword,
      );
      ...
      if (rawFilterValue.modId) {
        ...
        const rangeMatched = Array.from(resultsSet).filter((id) => {
          ...
          const matchedMod = getIndexedMods(indexerFilter.type, item).find(
            ({ id }) => id === modId,
          );
          return matchedMod
            ? minValue <= matchedMod.averageValue && maxValue >= matchedMod.averageValue
            : false;
        });
...
const addIndexerFilter = builder.dispatch((context, type: IndexerFilterType) => {
  ...
  const nextSerial = Math.max(
    ...currentFilters.filter((f) => f.type === type).map((f) => f.serial),
  );
 ...
 */
const removeIndexerFilter = builder.dispatch(
  (context, payload: { type: IndexerFilterType; serial: number }) => {
    filters.mutations.setIndexFilters(
      stateGetter().indexerFilters.filter(
        (f) => f.type !== payload.type || f.serial !== payload.serial,
      ),
    );
  },
  'removeIndexerFilter',
...
export const filters = {
  ...
  mutations: {
    setFilterValue: builder.commit(
      (state: FilterState, payload: { type: string; value: any; serial?: number }) => {
    ...
    setFilterEnabledDisabled: builder.commit(
      (state, payload: { type: string; value: boolean; serial?: number }) => {
        const filter = getFilter()(payload.type, payload.serial);
        if (filter) {
    ...
    resetFilterValues: builder.commit((state) => {
      [...state.indexerFilters, ...state.functionalFilters].forEach(
