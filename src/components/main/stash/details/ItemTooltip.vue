<template>
  <div class="item-tooltip">
    ...
    <section class="item-tooltip__content">
      ...
      <div v-for="enchant in enchantMods" :key="enchant" class="item-tooltip__mods--enchant">
        {{ enchant }}
      </div>

      <div v-for="impMod in implicitMods" :key="impMod" class="item-tooltip__mods--implicit">
        {{ impMod }}
      </div>
      <div :class="seperatorClass" v-if="hasExplicitBlock && hasImplicitBlock"></div>
      <div v-for="expMod in explicitMods" :key="expMod" class="item-tooltip__mods--explicit">
        {{ expMod }}
      </div>
      <div v-for="craftedMod in craftedMods" :key="craftedMod" class="item-tooltip__mods--enchant">
        {{ craftedMod }}
      </div>
      <div v-if="!this.item.identified" class="item-tooltip__font--corrupted">
        {{ unidedLabel }}
      </div>
      <div v-if="this.item.corrupted" class="item-tooltip__font--corrupted">
        {{ corruptedLabel }}
      </div>
    </section>
    <section>
    ...
    <section
      ...
      :key="`${gem.gemName}-${index}`"
      class="item-tooltip__font--gem"
    >
      {{ `${gem.gemName}  [${gem.level}/${gem.quality ? gem.quality : 0}%]` }}
    </section>
    <section v-if="influences" class="item-tooltip__font--influence">
      {{ influences }}
    </section>
  </div>
</template>
...
export default class ItemTooltip extends AppProps {
  ...
  private get hasExplicitBlock() {
    return this.explicitMods.length || this.item.corrupted || !this.item.identified;
  }

  private get hasImplicitBlock() {
    return this.implicitMods.length || this.fracturedMods.length || this.enchantMods.length;
  }
}
</script>
